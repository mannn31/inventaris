                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td><?= $row['jumlah']; ?></td>
                                <td>
                                    <a href="<?= base_url(); ?>/process/barang_keluar.php?act=<?= encrypt('delete'); ?>&idbarang_keluar=<?= encrypt($row['idbarang_keluar']); ?>" class="btn btn-sm btn-circle btn-danger btn-hapus"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group">
                                <?php
                                $query = mysqli_query($con, "SELECT max(idbarang_keluar) as maxId FROM barang_keluar");
                                $data = mysqli_fetch_array($query);
                                $id = $data['maxId'];
                                $urutan = (int) substr($id, 3, 3);
                                $urutan++;
                                $huruf = "BO";
                                $idbarang_keluar = $huruf . sprintf("%03s", $urutan);
                                ?>
                                <label for="idbarang_keluar">Id Barang Keluar <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="idbarang_keluar" name="idbarang_keluar" value="<?php echo $idbarang_keluar ?>" readonly required>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-5">
