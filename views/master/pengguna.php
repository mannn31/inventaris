...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <a href="#" class="btn btn-primary btn-icon-split btn-sm" data-toggle="modal" data-target="#penggunaModal" onclick="submit('add')">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td><?= $row['no_hp']; ?></td>
                                <td><?= $row['username']; ?></td>
                                <td><?php if ($row['level'] == "admin") {
                                        echo '<b>Administrator</b>';
                                    } else if ($row['level'] == "petugas") {
                                        echo '<b>Staff Pegawai</b>';
                                    }
                                    ?></td>
                                <td>
                                    <a href="#penggunaModal" data-toggle="modal" onclick="submit(<?= $row['id_users']; ?>)" class="btn btn-sm btn-circle btn-info"><i class="fas fa-edit"></i></a>
                                    <a href="<?= base_url(); ?>/process/users.php?act=<?= encrypt('delete'); ?>&id=<?= encrypt($row['id_users']); ?>" class="btn btn-sm btn-circle btn-danger btn-hapus"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
                    ...
                    <div class="row">
                        <div class="col-md-2">
                            <div class="form-group">
                                <?php
                                $query = mysqli_query($con, "SELECT max(id_users) as maxId FROM users");
                                $data = mysqli_fetch_array($query);
                                $id = $data['maxId'];
                                $urutan = (int) substr($id, 3, 3);
                                $urutan++;
                                $huruf = "Ad";
                                $id_users = $huruf . sprintf("%03s", $urutan);
                                ?>
                                <label>Id User</label>
                                <input type="hidden" name="id_users" class="form-control">
                                <input type="text" class="form-control" name="id_users" value="<?php echo $id_users ?>" readonly required>
                            </div>
                        </div>
                        <div class="col-md-10">
                            <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="level" class="form-control" required>
                                    ...
                                    <option value="petugas">Staff Pegawai</option>
                                </select>
                            </div>
                        </div>
                    </div>
